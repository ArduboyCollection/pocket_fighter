...
PROGMEM const uint8_t SPRITE_COPYRIGHT[] = {
  ...
  0x1c, 0x00, 0x3e, 0x00, 0x20, 0x00, 0x1c, 0x22, 0x1c, 0x00, 0x20, 0x00, 0x3e, 0x00, 0x00, 0x00
};
...
static void tickFighterShoot(Fighter* f) {
  ...
  if (f->dbx == 0) {
    if (++f->bx >= BORDER_RIGHT) finishFighterShoot(f);
    else if (++f->bx >= BORDER_RIGHT) finishFighterShoot(f);
  } else {
    if (--f->bx <= BORDER_LEFT) finishFighterShoot(f);
    else if (--f->bx <= BORDER_LEFT) finishFighterShoot(f);
  }
}
